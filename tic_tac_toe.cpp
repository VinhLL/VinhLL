...
// Output: Print current game state to screen
...
// Function: Detect next player (X or O)
// Input: Current game state
// Output: X or O
...
// Function: Change current game state
// Input: Current game state, move's coordinate (i and j), current player (X or O)
// Output: New game state after making the move
...
// Function: Find possible next game states
// Input: Current state
// Output: Next game states
...
// Functine: Check if current state is final state
// Input: Current state
// Output: True (if current state is final state)/ False (if current state is not final state)
...
// Function: Get final state's score
// Input: State (final)
// Output: Score (1 if X wins, -1 if O wins, 0 if draw)
...
// Function: Get score of current state
// Input: Current state
// Output: Best score can be reached from current state
...
// Function: Get coordinate of next computer's move
// Input: Current state (at computer's turn)
// Output: Next computer move's coordinate
...
int main() {
  ...
  while (true) {
    ...
    if (humanPlayer == currentPlayer) {
      ...
      j--;
      if ((i < 0 || i > 2) || (j < 0 || j > 2)) {
        cout << "Invalid move! Play it again." << endl;
        continue;
      }
      if (s[i][j] != ' ') {
        cout << "Invalid move! Play it again." << endl;
